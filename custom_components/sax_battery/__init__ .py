...
# Global variables for coordinating updates across all instances
GLOBAL_UPDATE_LOCK = asyncio.Lock()
LAST_SLAVE_40_UPDATE = 0
MIN_SLAVE_40_UPDATE_INTERVAL = 5  # Minimum seconds between batch updates for slave 40
# Cache for storing the most recent slave 40 batch read results
SLAVE_40_CACHE = {}
...
class SAXBattery:
    ...
    def __init__(self, hass, data, battery_id) -> None:
        ...
        self._update_lock = asyncio.Lock()  # Local lock for single battery updates
    ...
    async def async_update(self):
        """Update the battery data with enhanced throttling to prevent excessive modbus reads."""
        # First use a local lock to prevent concurrent updates to this battery
        async with self._update_lock:
            try:
                ...
                # First handle regular register updates (non-slave 40)
                for register_name, register_info in {k: v for k, v in registers.items() if v["slave"] != 40}.items():
                    # Skip if not due for update
                ...
                # Now handle slave 40 registers with global coordination
                slave_40_registers = {k: v for k, v in registers.items() if v["slave"] == 40}
                if not slave_40_registers:
                    return True

                # Check if any slave 40 registers need updating
                needs_update = any(
                    current_time - self._last_updates[reg_name] >= reg_info["scan_interval"]
                    for reg_name, reg_info in slave_40_registers.items()
                )

                if not needs_update:
                    return True

                # Now use the global lock to coordinate slave 40 reads across all instances
                async with GLOBAL_UPDATE_LOCK:
                    # Double-check throttling with global timestamp
                    time_since_global_update = current_time - LAST_SLAVE_40_UPDATE
                    if time_since_global_update < MIN_SLAVE_40_UPDATE_INTERVAL:
                        # Use cached values if available
                        battery_cache_key = f"{self.battery_id}_slave_40"
                        if battery_cache_key in SLAVE_40_CACHE:
                            cache_time, cache_data = SLAVE_40_CACHE[battery_cache_key]
                            # Use cached data if it's not too old (within 30 seconds)
                            if current_time - cache_time < 30:
                                for reg_name, value in cache_data.items():
                                    if reg_name in slave_40_registers:
                                        self.data[reg_name] = value
                                        self._last_updates[reg_name] = current_time
                                
                                _LOGGER.debug(
                                    "Using cached data for %s slave 40 (%.2f sec since last global update)",
                                    self.battery_id, time_since_global_update
                                )
                                return True
                        
                        # Log throttle and skip if no cache
                        _LOGGER.debug(
                            "Throttling slave 40 update for %s (%.2f sec since last global update)",
                            self.battery_id, time_since_global_update
                        )
                        return True

                    # Find the range of registers to read
                    start_address = min(reg_info["address"] for reg_info in slave_40_registers.values())
                    end_address = max(
                        reg_info["address"] + reg_info.get("count", 1) - 1
                        for reg_info in slave_40_registers.values()
                    )
                    register_count = end_address - start_address + 1

                    # Update the global timestamp BEFORE we do the actual read
                    GLOBAL_UPDATE_LOCK.locked()  # This doesn't do anything but keep the lock active
                    global LAST_SLAVE_40_UPDATE
                    LAST_SLAVE_40_UPDATE = current_time

                    _LOGGER.debug(
                        "Batch reading registers %s to %s (slave 40, count: %s)",
                        start_address, end_address, register_count
                    )
                    ...
                    try:
                        ...
                        battery_cache_key = f"{self.battery_id}_slave_40" 
                        SLAVE_40_CACHE[battery_cache_key] = (current_time, cache_data)

                    except (ConnectionError, TimeoutError, ValueError) as err:
                        _LOGGER.error("Failed to read bulk registers for slave 40: %s", err)
                        return False
