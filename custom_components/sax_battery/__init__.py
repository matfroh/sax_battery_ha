...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        if not _validate_config_entry(entry):
            raise ConfigEntryNotReady("Invalid configuration data")
        ...
        if not initialization_success:
            raise ConfigEntryNotReady("Failed to initialize battery connections")
        ...
        if not connected_batteries:
            raise ConfigEntryNotReady("No battery connections available")
        ...
        if not coordinators:
            raise ConfigEntryNotReady("No coordinators could be created")
